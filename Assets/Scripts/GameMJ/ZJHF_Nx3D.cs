...
public class ZJHF_Nx3D : MonoBehaviour ,IFCMjGameNet_Update{
    ...
    public void Init_()
    {
        ...
        GameObject uiGm = UIManager.Instance.ShowPerfabUI(AllPrefabName.uiWin_InGame_MJ, UIManager.Instance.canvas_T);
        InGameManage_3DMjUI inGM_UI = uiGm.GetComponent<InGameManage_3DMjUI>();
        ...
        uiW_ZJHF = UIManager.Instance.ShowUI(AllPrefabName.UiWin_ZJHF, UIManager.Instance.canvas_T).GetComponent<UiWin_ZJHF>();
    ...
    IEnumerator Play_ZJ()
    {
        ...
        for (int i = 0; i < zjhf_Fun_List.Count; i++)
        {
            ...
            {
                //Debug.Log(i + "看一下这是什么鬼！" + zjhf_Fun_List[i].ToString());
    ...
    void UpdateFeiPeng(uint charid, uint cardId, uint oriCharid)
    {
        ...
        */
        ...
        //    //Reset_ClickMJ();//初始化点击的麻将
        //    //yn_KeYiChuPai = true;//可以出牌
        ////}
        ////else
        ////{//非本地玩家
    ...
    public void UpdateMopai(uint charid, bool daChu, uint cardId)
    {
        Mj_Sx_ mjSx = cardId.ToCard();
        DataManage.Instance.MJSX_Add(mjSx);
    ...
    public string GOGogo(MJRoundRecord mrr_)
    {
        ...
        for (int i = 0; i < MRR.players.Count; i++)
        {
            ...
            Debug.Log("PlayerId:" + MRR.players[i].charId + "_ShouPai?__" + MRR.playBack.players[i].handCards.Count + "xxxxxxxxx");/* + MRR.playBack.players[i].handCards[0]);*/
            //strZhanJi += "PlayerStart_ShouPai" + MRR.players[i].charId + "|";
            for (int i1 = 0; i1 < MRR.playBack.players[i].handCards.Count; i1++)
            {
                ...
                //Gm_Manager.G_M.All_Resources.Add_Mj_Sx(Gm_Manager.G_M.All_Resources.ToCard(MRR.playBack.players[i].handCards[i1]));
                //strZhanJi += Gm_Manager.G_M.All_Resources.Get_Mj_Sx(MRR.playBack.players[i].handCards[i1]).mj_SpriteName + "|";
            }
            //strZhanJi += "\n";
            //int seat =  ;// GameManager.GM.GetPlayerNum(MRR.players[i].charId);

            int i_CIndex = i;
            zjhf_Fun_List.Add(delegate () {
                //Debug.Log("Index??"+ i_CIndex);
                List<uint> handCards_List = MRR.playBack.players[i_CIndex].handCards;
                handCards_List.Sort();
            ...
            //    //根据手牌数量确定庄家
            //    //Zhuangseat = seat;
            //    ZhuangID = MRR.players[i].charId;
            //    //PublicEvent.GetINS.Fun_reciveZhuang(MRR.players[i].charId);
        ...
        //strZhanJi += "ZhanJiHuiFang" + "\n";
        //var wait = new WaitForSeconds(0.1f);
        string mjCNName = "";
        strZhanJi = "";//战绩播放1
        //PublicEvent.GetINS.Event_reciveGetFirstCards();
        //int i_Delete1 = 0;
        ...
        while (!isjieshu)
        {
            while (isbofang)
            {
                ...
                {
                    //Debug.Log("出来的消息" + MRR.playBack.opInfo[Opnum].op + "传来的玩家" + MRR.playBack.opInfo[Opnum].charId + "传来的牌值" + MRR.playBack.opInfo[Opnum].card);
                    //输入牌指令
                    ...
                    switch (op)
                    {
                        ...
                        case ProtoBuf.MJGameOP.MJ_OP_MOPAI:
                            if (Cardid != 0)
                            {
                                ...
                                zjhf_Fun_List.Add(delegate () {
                                    ...
                                    UpdateMopai(Charid, true, Cardid);
                                    //Debug.Log("[ZhanJiHuiFang] UpdateMopai " + Cardid);
                                });
                                i_Count_HHH++;
                            }
                        ...
                        case MJGameOP.MJ_OP_CHUPAI:
                            ...
                            strZhanJi += str_2;
                            //strZhanJi += Zj_HuiFang_O_Type.o_ChuPai.ToString() + "|Id" + Charid + "|+CardId" + Gm_Manager.G_M.All_Resources.Get_Mj_Sx(Cardid).mj_SpriteName + "\n";
                            zjhf_Fun_List.Add(delegate () {
                                uiW_ZJHF.t_CLog.text = str_2;
                                UpdateChuPai(Charid, Cardid);
                                //Debug.Log("[ZhanJiHuiFang] UpdateChuPai " + Cardid);
                            });
                            i_Count_HHH++;
                            break;
                        ...
                        case ProtoBuf.MJGameOP.MJ_OP_PENG:
                            ...
                            strZhanJi += str_3;
                            //strZhanJi += Zj_HuiFang_O_Type.o_PengPai.ToString() + "|Id" + Charid + "|+CardId" + Gm_Manager.G_M.All_Resources.Get_Mj_Sx(Cardid).mj_SpriteName + "\n";
                            zjhf_Fun_List.Add(delegate () {
                                uiW_ZJHF.t_CLog.text = str_3;
                                UpdatePengPai(Charid, Cardid);
                                //Debug.Log("[ZhanJiHuiFang] UpdatePengPai " + Cardid);
                            });
                            i_Count_HHH++;
                            break;
                        case ProtoBuf.MJGameOP.MJ_OP_GANG:
                            ...
                            strZhanJi += str_4;
                            //strZhanJi += Zj_HuiFang_O_Type.o_MingGang.ToString() + "|Id" + Charid + "|+CardId" + Gm_Manager.G_M.All_Resources.Get_Mj_Sx(Cardid).mj_SpriteName + "\n";
                            zjhf_Fun_List.Add(delegate () {
                                uiW_ZJHF.t_CLog.text = str_4;
                                UpdateGangPai(Charid, Cardid, Oricharid);
                                //Debug.Log("[ZhanJiHuiFang] UpdateGangPai " + Cardid);
                        ...
                        case ProtoBuf.MJGameOP.MJ_OP_HU:
                            ...
                            //strZhanJi += Zj_HuiFang_O_Type.o_HuPai.ToString() + "|Id" + Charid + "|+CardId" + Gm_Manager.G_M.All_Resources.Get_Mj_Sx(Cardid).mj_SpriteName + "\n";
                            zjhf_Fun_List.Add(delegate () {
                                uiW_ZJHF.t_CLog.text = str_5;
                                UpdateHule(Charid, Cardid, Oricharid);
                                //Debug.Log("[ZhanJiHuiFang] UpdateHule " + Cardid);
                            });
                            i_Count_HHH++;
                            break;
                        case ProtoBuf.MJGameOP.MJ_OP_TING:
                            Debug.Log("有玩家 听牌？？？？？？——");
                            //strZhanJi += Zj_HuiFang_O_Type.o_TingPai.ToString() + "|Id" + Charid + "|+CardId" + Gm_Manager.G_M.All_Resources.Get_Mj_Sx(Cardid).mj_SpriteName + "\n";
                            break;
                        ...
                        case ProtoBuf.MJGameOP.MJ_OP_FEI:
                            //有玩家飞碰
                            mjCNName = MJHs_ToCN(DataManage.Instance.MJSX_Get(Cardid).mj_SpriteName);
                            string str_6 = i_Count_HHH + "<color=red>【玩家飞碰】</color>" + "名字：" + DataManage.Instance.PData_GetData(Charid).P_Name + "—飞碰：" + mjCNName + "\n";
                            strZhanJi += str_6;

                            //strZhanJi += Zj_HuiFang_O_Type.o_HuPai.ToString() + "|Id" + Charid + "|+CardId" + Gm_Manager.G_M.All_Resources.Get_Mj_Sx(Cardid).mj_SpriteName + "\n";
                            zjhf_Fun_List.Add(delegate () {
                                uiW_ZJHF.t_CLog.text = str_6;
                                UpdateFeiPeng(Charid, Cardid, Oricharid);
                                //Debug.Log("[ZhanJiHuiFang] UpdateHule " + Cardid);
                            });
                            i_Count_HHH++;
                            break;
                        case ProtoBuf.MJGameOP.MJ_OP_TI:
                            ...
                            mjCNName = MJHs_ToCN(DataManage.Instance.MJSX_Get(Cardid).mj_SpriteName);
                            string str_7 = i_Count_HHH + "<color=red>【玩家提牌】</color>" + "名字：" + DataManage.Instance.PData_GetData(Charid).P_Name + "—提牌：" + mjCNName + "\n";
                            strZhanJi += str_7;

                            //strZhanJi += Zj_HuiFang_O_Type.o_HuPai.ToString() + "|Id" + Charid + "|+CardId" + Gm_Manager.G_M.All_Resources.Get_Mj_Sx(Cardid).mj_SpriteName + "\n";
                            zjhf_Fun_List.Add(delegate () {
                                uiW_ZJHF.t_CLog.text = str_7;
                                UpdateTiPai(Charid, Cardid);
                                //Debug.Log("[ZhanJiHuiFang] UpdateHule " + Cardid);
                            });
                            i_Count_HHH++;
                            break;
