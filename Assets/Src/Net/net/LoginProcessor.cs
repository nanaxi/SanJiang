            ...
            if (!m_connection.IsConnected())
            {
                    ...
                    Time.time - m_lastReconnectTime > m_reconnectInterval)
                {
                    if (Time.time - m_breakConnectTime > m_maxReconnectTime)
                    {
                        ...
                        //GameManager.GM.connectText.text += "Please ReLogin....";
                        //GameManager.GM.connectText.transform.SetAsLastSibling();
...
//----------------------------------------------------------------------------------------------------------------
    ...
    public bool CheckVersionResponse(CheckVersionRsp rsp)
    {
        ...
        {
            UnityEngine.Debug.Log("检查客户端版本及搜集玩家手机信息__ELSE???");
    ...
    public bool AccountLoginRequest()
    {
        ...
        UnityEngine.Debug.Log(GlobalSettings.LoginUserId+"申请登录" + GlobalSettings.LoginUserName);
        return retCode;
    }
    ...
    public bool AccountLoginResponse(AccountLoginRsp rsp)
    {
        ...
        switch (rsp.result)
        {
            ...
            default:
                ...
                //PublicEvent_.GetINS.Fun_LoginResult(rsp);
